...
def validate_defined_c_steps(
    ...
    *,
    ...
    """Raise on a large C step where the surface heading is well defined.

    Samples with ``|n_xy| < nxy_min`` are singular: C is held and a later
    jet-off slew absorbs the catch-up, so those steps are not errors.
    """
    c = np.asarray(c_angles, dtype=float)
    nxy = np.hypot(normals[:, 0], normals[:, 1])
    threshold = float(max_c_step_deg)
    pole = float(nxy_min)
    for i in range(1, len(c)):
        if nxy[i] < pole or nxy[i - 1] < pole:
            continue
        step = c_step_deg(c[i - 1], c[i])
        if step > threshold:
            raise ValueError(
                f"C-axis step {step:.1f} deg at index {i} "
                f"(C {c[i - 1]:.2f} -> {c[i]:.2f}) "
...
def hold_c_near_pole(
    c_angles: np.ndarray,
    normals: np.ndarray,
    *,
    nxy_min: float,
) -> np.ndarray:
    """Keep the last reliable C while the normal's horizontal part vanishes.

    At the crown C is a twist about a vertical nozzle and does not aim the
    tool. Tracking ``atan2(ny, nx)`` there swings the arm without moving the
    deposit. A trace that starts in the singular region takes C from the first
    sample where ``|n_xy|`` is at least ``nxy_min``.
    """
    c = np.asarray(c_angles, dtype=float).copy()
    if len(c) == 0 or nxy_min <= 0.0:
        return c
    nxy = np.hypot(normals[:, 0], normals[:, 1])
    reliable = np.flatnonzero(nxy >= float(nxy_min))
    if len(reliable) == 0:
        c[:] = c[0]
        return c

    held = float(c[reliable[0]])
    for i in range(len(c)):
        if nxy[i] < float(nxy_min):
            c[i] = held
        else:
            held = float(c[i])
    return c
